...
if menu == "📋 頁面一：Full Route & 即時狀態":
    ...
    # ---- 💡 中段路由大表處理區塊 (若雲端無資料則只顯示友善提示，絕不拖累下段) ----
    if route_df is not None and not route_df.empty:
        ...
        if not full_route_df.empty:
            ...
            event = st.dataframe(full_route_df, use_container_width=True, height=300, selection_mode="single-row", on_select="rerun", hide_index=True)
    ...
    # ---- 💡 核心修正：將（下）方數據完全从小條件中解放，強行放在最外層，確保永久顯示！ ----
    st.markdown("---")
    ...
    # 如果同仁滑鼠點選了表格中的特定站點，下方卡片指標會隨著點擊站點動態更新跳動
    if st.session_state.selected_row_data is not None:
        ...
        if route_df is not None and not route_df.empty:
            meta_match = route_df[route_df["Step No."].astype(int) == int(current_step_val)] if "Step No." in route_df.columns else pd.DataFrame()
